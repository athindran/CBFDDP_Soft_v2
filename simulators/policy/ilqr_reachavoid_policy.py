...
class iLQRReachAvoid(iLQR):
    ...
    def run_ddp_iteration(self, args):
        states, controls, J, critical, _, _, _, _, _, _, _, _, _, num_iters, warmup = args
        ...
        num_iters += 1
        return (states, controls, J_new, critical, reachavoid_margin, 
    ...
    def check_ddp_iteration_continue(self, args):
        _,  _,  J_new,  _,  _,  _,  _, status, _, _, _, _, _, num_iters, warmup = args
    ...
    def get_action_jitted(
        ...
        num_iters = 0
        (states, controls, J, critical, reachavoid_margin, alpha_chosen, 
                cvg_tolerance, status, V_x, V_xx, fu, k_open_loop, K_closed_loop, num_iters, warmup) = jax.lax.while_loop(check_ddp_iteration_continue, run_ddp_iteration, 
                                            (states, controls, J, critical, reachavoid_margin,
                                        alpha_chosen, 1.0, 0, c_x[:, 0], c_xx[:, :, 0], fu, k_open_loop, K_closed_loop, num_iters, warmup))

        # solver_info = dict(
        #     states=states, controls=controls, reinit_controls=controls, t_process=0.0, status=status, Vopt=J, marginopt=reachavoid_margin,
        ...
        return controls[:, 0], controls, states, reachavoid_margin, J, False, V_x
