...
class Bicycle5D(BaseDynamics):
    ...
    def __init__(self, config: Any, action_space: np.ndarray) -> None:
        ...
        self.noise_var = jp.array([0.01, 0.01, 0.01, 0.01, 0.01])
    ...
    def disc_deriv(
        ...
        deriv = deriv.at[3].set(state[2] * jp.tan(state[4]) / self.wheelbase)
    ...
    def _integrate_forward_dt(
        ...
        state_nxt = state_nxt.at[4].set(
            jp.clip(state_nxt[4], self.delta_min, self.delta_max)
        )
    ...
    def get_jacobian_fx(
        ...
        Ac = jp.array([[0, 0, jp.cos(obs[3]), -obs[2] * jp.sin(obs[3]), 0],
                        [0, 0, jp.sin(obs[3]), obs[2] * jp.cos(obs[3]), 0],
                        [0, 0, 0, 0, 0],
                        [0,
                         0,
                         jp.tan(obs[4]) / self.wheelbase,
                         0,
                         obs[2] / (1e-6 + self.wheelbase * jp.cos(obs[4])**2)],
                        [0, 0, 0, 0, 0]])
